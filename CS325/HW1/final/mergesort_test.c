...
*/
    ...
    if((outputFile=fopen("merge_test.out","w"))==NULL){
 ...
 **************************************/
int mergesort(int* arrIn, int length){
    int middle,i,rsize,lsize,right_count=0;
    int* left=NULL;
    int* right=NULL;
    ...
    if(length==1)
        ...
        rsize=length-middle;
        lsize=middle;
        left=(int*) malloc(sizeof(int) * lsize);            //create a temp left array
        right=(int*) malloc(sizeof(int) * rsize+1);  //create a temp right array
